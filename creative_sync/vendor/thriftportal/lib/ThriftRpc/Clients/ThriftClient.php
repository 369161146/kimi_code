...
class ThriftInstance
{
    ...
    public function __call($method_name, $arguments)
    {
        ...
        if (0 === strpos($method_name, self::ASYNC_SEND_PREFIX)) {
            ...
            if (isset($this->thriftAsyncInstances[$method_name_key])) {
                ...
                throw new \Exception($this->serviceName . "->$method_name(" . implode(',', $arguments) .
                                     ") already has been called, you can't call again before you call " .
                                     self::ASYNC_RECV_PREFIX . $real_method_name, 500);
            }
        ...
        if (0 === strpos($method_name, self::ASYNC_RECV_PREFIX)) {
            ...
            // 判断是否有发送过这个方法的异步请求
            if (!isset($this->thriftAsyncInstances[$method_name_key])) {
                throw new \Exception($this->serviceName . "->$send_method_name(" . implode(',', $arguments) .
                                     ") have not previously been called", 500);
            }
    ...
    protected function __instance()
    {
        ...
        $socket->setSendTimeout(120 * 1000);
